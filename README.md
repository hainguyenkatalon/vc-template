...
Run `./create_instance.sh <folder-name>` to copy every file from `template/` into a brand-new sibling folder. Supplying the folder name is mandatory, and the script aborts if the destination already exists with content. After copying, the script prompts for a repo path to append to `REPOS.md` and reminds you to fill in `SPEC.md`. The script deliberately skips `README.md` and itself so that downstream repos only contain the spec documents.
